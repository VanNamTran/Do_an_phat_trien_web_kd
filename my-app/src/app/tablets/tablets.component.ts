...
export class TabletsComponent {
  ...
  ngOnInit() {
    ...
    this.brands = ['samsung', 'coolpad', 'masstel', 'Apple (ipad)', 'lenovo', 'oppo', 'xiaomi'];
  ...
  onChange(event:any){
    if (event.target.checked){
      this.temTablets = this.arrays.filter((phone: any)=>phone.brand == event.target.value)
      this.tabletProducts = []
      this.newTablets.push(this.temTablets)
      for (let i=0; i<this.newTablets.length; i++){
        var firstPhone = this.newTablets[i];
        for (let i=0; i<firstPhone.length; i++){
          var obj = firstPhone[i];
          this.tabletProducts.push(obj);
        }
      }
    }
    else{
      this.getProducts();
      this.temTablets = this.tabletProducts.filter((phone: any)=>phone.brand != event.target.value)
      this.newTablets = []
      this.tabletProducts = []
      this.newTablets.push(this.temTablets)
      for (let i=0; i<this.newTablets.length; i++){
        var firstTab = this.newTablets[i]
