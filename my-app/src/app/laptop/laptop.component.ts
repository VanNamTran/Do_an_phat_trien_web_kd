...
export class LaptopComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.brands = ['msi', 'asus', 'lenovo', 'hp', 'dell', 'gigabyte', 'microsoft', 'acer', 'apple'];
  ...
  onChange(event:any){
    if (event.target.checked){
      this.temLaptops = this.arrays.filter((phone: any)=>phone.brand == event.target.value)
      this.laptopProducts = []
      this.newLaptops.push(this.temLaptops)
      for (let i=0; i<this.newLaptops.length; i++){
        var firstLaptop = this.newLaptops[i];
        for (let i=0; i<firstLaptop.length; i++){
          var obj = firstLaptop[i];
          this.laptopProducts.push(obj);
        }
      }
    }
    else{
      this.getProducts();
      this.temLaptops = this.laptopProducts.filter((phone: any)=>phone.brand != event.target.value)
      this.newLaptops = []
      this.laptopProducts = []
      this.newLaptops.push(this.temLaptops)
      for (let i=0; i<this.newLaptops.length; i++){
        var firstLap = this.newLaptops[i]
        for (let i=0; i<firstLap.length; i++){
          var obj = firstLap[i]
          this.laptopProducts.push(obj)
        }
      }
