...
export class WatchComponent {
  ...
  onChange(event:any){
    if (event.target.checked){
      this.temWatches = this.arrays.filter((phone: any)=>phone.brand == event.target.value)
      this.watchProducts = []
      this.newWatches.push(this.temWatches)
      for (let i=0; i<this.newWatches.length; i++){
        var firstPhone = this.newWatches[i];
        for (let i=0; i<firstPhone.length; i++){
          var obj = firstPhone[i];
          this.watchProducts.push(obj);
        }
      }
    }
    else{
      this.getProducts();
      this.temWatches = this.watchProducts.filter((phone: any)=>phone.brand != event.target.value)
      this.newWatches = []
      this.watchProducts = []
      this.newWatches.push(this.temWatches)
      for (let i=0; i<this.newWatches.length; i++){
        var firstWatch = this.newWatches[i]
        for (let i=0; i<firstWatch.length; i++){
          var obj = firstWatch[i]
          this.watchProducts.push(obj)
        }
      }
    }
